...
export default function App() {
	...
	// Handle search of characters
	const [searchCharacter, setSearchCharacter] = useState("");
	const previousSearchCharacter = usePrevious(searchCharacter);

	// Cache the data for faster rendering during pages switching
	window.onload = () => {
	...
	// Handle pagination and previous searchs
	useEffect(() => {
	...
	// Functions to handle displayed pages
	...
	if (isFetching) {
		return (
			<div className="App">
				<StationaryComponets isLoading={isFetching} changePage={changePage} handleSearch={(searched) => setSearchCharacter(searched)} />
		...
		return (
			<div className="App">
				<StationaryComponets onSearch={searchCharacter !== ""} changePage={changePage} handleSearch={(searched) => setSearchCharacter(searched)} backToPage={backToPage} />
				{searchCharacter === "" ? <h4 style={{ color: "#fee71e", marginTop: 20 }}>Page: {pageNumber}</h4> : null}
				<Table
					rows={characters.map((person) => (
						<TableDataRow character={person} key={person.name} />
					))}
				/>
			</div>
		);
